...
class Student {
    ...
    // знакомство, установление программ,

    // простые типы данных и строки, пулл строк и пулл интеджеров

    //- ооп, инкапсуляция, наследование, полиморфизм
    //- геттеры сеттеры

    //создать класс профессор и наследовать от человека и определить метод сэй хеллоу hellowWithHuman(Human human), передать студента человека и профессора
    //массив и ввылку можно ли его поменять
    //Аргумент - это значение, которое передается методу при его вызове.
    //Если у студента есть иетод сходитьна пару... мы не сможем его вызвать goToClass()

    //пометить метод как finall getControllSumm - override? это означает что переопделять его нельзя

    //immutable - ?
    //если студент класса хьюман мы можем какими методами пользоваться?
